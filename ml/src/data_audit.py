...
Scans the raw Dataset/ directory and reports:
  - Total image and label file counts per survey year.
  - Which images have a corresponding YOLO annotation (labeled) vs. background only.
  - Per-class annotation counts.
  - Any label format anomalies (wrong class id, out-of-range bbox coordinates).

Usage (from project root):
    python ml/src/data_audit.py
...
def _parse_yolo_label(
    ...
    Returns:
        ...
        errors:      List of human-readable format error strings.
    """
    ...
    if not text:
        return annotations, errors  # Empty = background tile

    for line_no, raw in enumerate(text.splitlines(), start=1):
        parts = raw.strip().split()
        if len(parts) != 5:
            errors.append(
                f"{label_path.name}:{line_no} -- expected 5 fields, got {len(parts)}"
            )
            continue
        try:
            cls = int(parts[0])
        ...
        for val, name in ((cx, "cx"), (cy, "cy"), (bw, "bw"), (bh, "bh")):
            if not (0.0 <= val <= 1.0):
                errors.append(
                    f"{label_path.name}:{line_no} -- {name}={val:.4f} out of [0, 1]"
                )
...
def audit_directory(survey_dir: Path) -> Dict:
    """Audit a single survey directory (e.g. Dataset/2010).

    Returns a dict summarising counts and anomalies.
    """
    if not survey_dir.is_dir():
        raise FileNotFoundError(f"Survey directory not found: {survey_dir}")

    image_files = sorted(
        p for p in survey_dir.iterdir() if p.suffix.lower() in IMAGE_EXTENSIONS
    )
    ...
    for img in image_files:
        ...
        if lbl_path is None:
            images_without_label.append(img.name)
            background_images += 1
            continue

        annotations, errors = _parse_yolo_label(lbl_path)
...
def audit_dataset(dataset_root: Path) -> Dict:
    """Audit all survey sub-directories under dataset_root."""
    ...
    for sdir in survey_dirs:
        report = audit_directory(sdir)
        per_survey.append(report)
        grand_total_images += report["total_images"]
        grand_labeled += report["labeled_images"]
        grand_background += report["background_images"]
        grand_annotations += report["total_annotations"]
        total_errors += report["format_error_count"]
        for cls_name, count in report["class_counts"].items():
            grand_class_counts[cls_name] = grand_class_counts.get(cls_name, 0) + count

    return {
        "dataset_root": str(dataset_root.resolve()),
        "surveys_found": [d.name for d in survey_dirs],
        "summary": {
            "total_images": grand_total_images,
            "labeled_images": grand_labeled,
            "background_images": grand_background,
            "total_annotations": grand_annotations,
            "class_counts": grand_class_counts,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit the raw sonar dataset and report image/annotation statistics."
    )
    parser.add_argument(
        "--dataset-root", "-d",
        type=str,
        default="Dataset",
        help="Path to the dataset root containing per-year survey sub-dirs (default: Dataset).",
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
...
def main() -> None:
    ...
    print(f"\n[data_audit] Scanning dataset at: {dataset_root.resolve()}\n")
    ...
    print(f"  Format errors    : {s['total_format_errors']}")
    print("=" * 56)

    for srv in report["per_survey"]:
        print(
            f"\n  [{srv['survey']}]  "
            f"{srv['total_images']} images  |  "
            f"{srv['labeled_images']} labeled  |  "
            f"{srv['background_images']} background  |  "
            f"{srv['total_annotations']} annotations"
        )
        for cls_name, cnt in srv["class_counts"].items():
            print(f"      {cls_name}: {cnt}")
        if srv["format_errors"]:
            print(f"      WARNING: {srv['format_error_count']} format error(s):")
            for err in srv["format_errors"]:
                print(f"        - {err}")
